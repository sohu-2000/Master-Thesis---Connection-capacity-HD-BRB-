...
d1_screws = 3                                   # mm, inner diameter of screws

d_head_screws = 7.8                                # mm, diameter of screw head
...
k_p = 0.7                                              # Table 11.2 EN1995-1-1:2025, value for perpendicular
...
F_t_k = 7900                                                # N from ETA-11/0030 of 2025/08/20

f_u_k =  F_t_k/As                                           # Based on F_t_k from ETA f_u_k = F_t_k/As

M_y_k = 0.3 * f_u_k * 0.86 * d1_screws**2.6                 # Nmm, Table 11.7 EN1995-1-1:2025
...
f_h2_k = (0.082 * p_k * d_screws**-0.3)                # N/mm^2, Table 11.6 EN1995-1-1:2025 - for perpendicular screws gives 1 in denominator
...
# Steel tensile resistance, ETA-11/0030 of 2025/08/20
...
n = 20
n_eff = max(n**0.9, 0.9*n)                                       # Effective number of fastners for LBS screws, from ETA-23/0813

F_vd = kmod*n_eff*F_vk/gamma_r                      # EC5 11.3.3(3) Splitting resistance

# Resistance of steel plate

F_1_steel_Rk = 40000                                #N  Characteristic load-carrying capacity of the hold-down, from ETA-23/0813
...
capacities = {
    ...
    "Fastener group lateral capacity F_vd": F_vd
}
